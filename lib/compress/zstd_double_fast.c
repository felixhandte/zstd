        ...
        void const* src, size_t srcSize, U32 const mls /* template */)
{
    ...
    while (ip < ilimit) {  /* i.e., ip1 <= ilimit */
        size_t step = 1;
        const BYTE* nextStep = ip + kStepIncr;
        const BYTE* ip1 = ip + step;

        size_t hl0 = ZSTD_hashPtr(ip, hBitsL, 8);
        size_t hl1;

        U32 idxl0 = hashLong[hl0];
        U32 idxl1;

        const BYTE* matchl0 = base + idxl0;
        const BYTE* matchl1;
        const BYTE* matchs0;
...
_match_found: /* requires ip, offset, mLength */
        ...
        if (step < 4) {
            /* It is unsafe to write this value back to the hashtable when ip1
             * is greater than or equal to the new ip we will have after we're
             * done processing this match. Rather than perform that test
             * directly (ip1 >= ip + mLength), which costs speed in practice,
             * we do a simpler more predictable test. The minmatch even if we
             * take a short match is 4 bytes, so as long as step, the distance
             * between ip and ip1 (initially) is less than 4, we know
             * ip1 < new ip. */
